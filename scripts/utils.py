...
This .py file contains the following functions:
	...
	get_translation(nmt_model, sentence, lang_obj, use_cuda, source_name = 'vi', target_name = 'en')
"""
...
def get_binned_bl_score(nmt_model, val_dataset, location, batchSize):
	...
	for i in range(1, len(len_threshold)):
		...
		temp_loader = DataLoader(temp_dataset, batch_size = batchSize, collate_fn = partial(dataset_helper.vocab_collate_func), shuffle = True, num_workers=0)
...
def get_translation(nmt_model, sentence, lang_obj, use_cuda, source_name = 'vi', target_name = 'en'):
	from googletrans import Translator
