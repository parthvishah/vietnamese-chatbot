...
#         att_score = (encoder_outs.transpose(0, 1) * x.unsqueeze(0)).sum(dim=2)
		...
		seq_mask = self.sequence_mask(src_lens, max_len=max(src_lens).item(), device = hidden.device).transpose(0, 1)
...
class Decoder_SelfAttn(nn.Module):
	...
	def __init__(self, output_size, hidden_size, idropout=0.5, self_attention = False, encoder_attention = False):
		...
		self.memory_rnn = nn.GRUCell(hidden_size + int(self.encoder_attention==True)*self.hidden_size, hidden_size, bias=True);
...
class seq2seq(nn.Module):
	...
	def train_step(self, batch):
		...
		decoder_output, decoder_hidden, _, _ = self.decoder(decoder_input, encoder_hidden, encoder_output, xs_len)
