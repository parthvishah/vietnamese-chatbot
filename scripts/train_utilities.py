...
def train_model(encoder_optimizer,decoder_optimizer, encoder, decoder, loss_fun,m_type, dataloader, en_lang,vi_lang,save_path,enc_pth_name,dec_pth_name,\
                num_epochs=60, val_every = 1, train_bleu_every = 10,clip = 0.1, rm = 0.8, enc_scheduler = None,\
               dec_scheduler = None, enc_dec_fn = encode_decode, val_fn = validation_new):
    best_score = 0
    best_bleu = 0
    ...
    for epoch in range(num_epochs):
        ...
        if epoch%val_every == 0:
            ...
            if val_bleu_score > best_bleu:
                ...
                utils.save_models(best_encoder_wts, save_path, enc_pth_name)
                utils.save_models(best_decoder_wts, save_path, dec_pth_name)
            log.info(f"epoch {epoch} | loss {epoch_loss} | time = {time.time() - start} | validation bleu = {val_bleu_score}")
        print('='*50)
    # encoder.load_state_dict(best_encoder_wts)
    # decoder.load_state_dict(best_decoder_wts)
    log.info("Training completed. Best BLEU is {}".format(best_bleu))
